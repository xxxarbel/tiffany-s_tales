...
// AI book suggestions for a member — the latest set generated by Tiffany's
// concierge (claude-haiku-4-5 + web search) from the member's reading profile
// and rated log. One row per member (PK = userId, 1:1 with `user`); the picks
// are stored as a JSON-encoded `Suggestion[]` in a text column (this schema uses
// plain text for all rich fields — no jsonb). Regenerated on demand via the
// "Get suggestions" / "Refresh" button. Cascades away with the account.
export const aiSuggestions = pgTable("ai_suggestions", {
  userId: text("user_id")
    .primaryKey()
    .references(() => user.id, { onDelete: "cascade" }),
  suggestions: text("suggestions"), // JSON-encoded Suggestion[]
  model: text("model"),
